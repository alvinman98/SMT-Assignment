...
class CashSettlementPayment(models.Model):
    _name = 'cash.settlement.payment'
    _description = 'Cash Settlement Payment'

    advance_id = fields.Many2one('account.cash.settlement', string="Settlement To Pay", required=True, default=lambda self: self._context.get('active_id', False))
    partner_id = fields.Many2one('hr.employee', string="Partner", related='advance_id.employee_id')
    journal_id = fields.Many2one('account.journal', string="Payment Journal", required=True)
    amount_paid = fields.Float(string="Payment Amount", required=True, compute='_compute_amount_paid')
    ...
    @api.depends('advance_id.total_pay')
    def _compute_amount_paid(self):  
        """Compute the amount paid based on the advance's total pay amount."""  
        for record in self:  
            record.amount_paid = record.advance_id.total_pay if record.advance_id else 0.0  
    ...
    def _onchange_advance_id(self):  
        ...
        if self.advance_id:  
            self.amount_paid = self.advance_id.total_pay  
            self.currency = self.advance_id.currency  

    def action_validate_register_payment(self):  
        """Validate the cash settlement payment and create accounting entries."""  
        self.amount_paid = self.advance_id.total_pay  
    ...
    def _prepare_move_value(self, move_lines):  
        ...
        return {  
            ...
            'settlement_payment_id': self.advance_id and self.advance_id.id or False,  
            'line_ids': move_lines,  
        }
